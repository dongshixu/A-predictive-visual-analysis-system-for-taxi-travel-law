    ...
    var extensionMethods = {
        ...
        float: function( settings ) {
            ...
            var options = {
                ...
                formatLabel: function(value) {
                    var hh = this.prefix + value + this.suffix;
                    var hhh = hh.split(".");
                    return hhh[0]
                }
                // function
            ...
            if ( options.handle ) {
                ...
                // else if its just a normal slider
                    ...
                    $tip = $("<span class=\"ui-slider-tip\">00:00</audio></span>");
...
//              console.log(typeof(val[0]))
                var test_dot = String(val[0]).indexOf(".") + 1;//获取小数点的位置
			    var count = String(val[0]).length - test_dot;//获取小数点后的个数
			    var showTime, hour, mins
			    if(test_dot > 0) {
//			        alert("这个数字是小数，有" + count + "位小数");
			        var str = val[0].toString().split('.')
			        if(parseInt(str[0]) < 10){
			        	hour = "0"+str[0];
			        }else{
			        	hour = str[0];
			        };
			        if(parseInt(str[1]) == 16667){
			        	mins = 10;
			        }else if(parseInt(str[1]) == 33333){
			        	mins = 20;
			        }else if(parseInt(str[1]) == 5){
			        	mins = 30;
			        }else if(parseInt(str[1]) == 66667){
			        	mins = 40;
			        }else {
			        	mins = 50;
			        };
...
//              $(ui.handle).find(".ui-slider-tip").html( options.formatLabel( val[0] ) );
                $(ui.handle).find(".ui-slider-tip").html(showTime);
